...
class AdminController {
   ...
   // 3. Form sửa học phần
    public function course_edit() {
        ...
        if (isset($_GET['id'])) {
            $model = new CourseModel();
            // Lấy thông tin học phần theo ID (mã HP)
            $course = $model->getCourseById($_GET['id']); 
            
            if ($course) {
                // Gọi view sửa học phần
                require_once __DIR__ . '/../../views/admin/course_edit.php';
            } else {
                 echo "<script>alert('Không tìm thấy học phần!'); window.history.back();</script>";
            }
        }
    }
    ...
    if (isset($_GET['id'])) {
        ...
        // SỬA: Đổi tên biến thành $advisor để khớp với View
        $advisor = $model->getAdvisorById($_GET['id']);
        
        if ($advisor) { // Sửa cả chỗ kiểm tra này
...
public function class_edit() {
    ...
    if (isset($_GET['id'])) {
        ...
        // SỬA: Đổi tên biến thành $class để khớp với View
        $class = $model->getClassById($_GET['id']); 
        
        $advModel = new AdvisorModel();
        $dsCoVan = $advModel->getAllAdvisors();
        
        if ($class) { // Sửa cả chỗ kiểm tra này
